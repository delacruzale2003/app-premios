...
const conectarDB = require('../db');  // Conexión a la base de datos

module.exports = async (req, res) => {
    await conectarDB();  // Conectar a la base de datos antes de manejar la solicitud

    if (req.method === 'POST' && req.url === '/api/premio/crear') {
        return await crearPremio(req, res);
    }

    if (req.method === 'PUT' && req.url === '/api/premio/entregar') {
        return await entregarPremio(req, res);
    }

    if (req.method === 'GET' && req.url.startsWith('/api/premio/tienda')) {
        return await getPremiosByIdTienda(req, res);
    }
...
const entregarPremio = async (req, res) => {
    ...
    try {
        ...
        // Restar 1 al stock disponible del premio
        ...
        // Buscar al cliente y asignarle el premio
...
// Función para obtener los premios por ID de tienda
const getPremiosByIdTienda = async (req, res) => {
